...
// Octicons (https://primer.style/octicons/) — same icons GitHub uses.
const ALERT_ICONS: Record<AlertType, Element> = {
  note: octicon(
...
function octicon(path: string): Element {
  return {
    type: "element",
    tagName: "svg",
    properties: {
...
function detectAlertType(blockquote: Element): {
  type: AlertType;
  paragraph: Element;
} | null {
  const firstChild = blockquote.children.find(isElement);
...
function stripAlertMarker(paragraph: Element): void {
  ...
  // Drop a leading <br> and surrounding whitespace left from the marker line.
  if (firstText.value === "") {
    paragraph.children.shift();
    const next = paragraph.children[0];
    if (isElement(next) && next.tagName === "br") {
      paragraph.children.shift();
    }
    const after = paragraph.children[0];
    if (after?.type === "text") {
...
const rehypeGithubAlerts: Plugin<[], Root> = () => {
  return (tree: Root) => {
    visit(tree, "element", node => {
      ...
      const detected = detectAlertType(node);
      if (!detected) {
        return;
      }

      stripAlertMarker(detected.paragraph);
      ...
      const title: Element = {
        ...
        children: [
          ALERT_ICONS[detected.type],
          { type: "text", value: ALERT_TITLES[detected.type] },
        ],
      };

      node.tagName = "div";
      node.properties = {
        className: ["markdown-alert", `markdown-alert-${detected.type}`],
      };
